// Lettura e interpretazione del calendario Google condiviso di permessi e ferie.
// Unico punto in cui si scarica l'ICS e si ricavano nome, tipo e ore dal titolo:
// lo usano sia la lettura su richiesta sia la sincronizzazione nel database.
...
export async function leggiCalendario(forza = false) {
...
export function parseDateValue(fullKey, val) {
...
export function addOneDay(yyyy_mm_dd) {
...
export function covers(event, target) {
...
// Permessi e ferie di un singolo giorno, senza doppioni per persona e tipo.
export function permessiDelGiorno(events, giorno) {
  const result = [];
  const seen = new Set();
...
// Elenco di permessi per ogni giorno dell'intervallo: { "yyyy-MM-dd": [...] }
export function permessiDelPeriodo(events, da, a) {
  const perGiorno = {};
  let giorno = da;
  let guardia = 0;
  while (giorno <= a && guardia < 62) {
    const lista = permessiDelGiorno(events, giorno);
    if (lista.length) perGiorno[giorno] = lista;
